...
sub PythonType($$$$)
{
	...
	if ($actual_ctype->{TYPE} eq "UNION" and defined($actual_ctype->{ELEMENTS})) {
		...
		$self->deindent;
		$self->pidl("}");
		$self->pidl("return Capsule_FromVoidPtrAndContext(out, discard_const_p(char, out_type), NULL);");
		$self->deindent;
		$self->pidl("}");
		$self->pidl("");
...
sub ConvertObjectToPythonLevel($$$$$$)
{
	...
	if ($l->{TYPE} eq "POINTER") {
		...
		if ($conv =~ /^pytalloc_CObject_FromTallocPtr/) {
			(my $conv_with_pycapsule = $conv) =~ s/pytalloc_CObject_FromTallocPtr/pytalloc_PyCapsule_FromTallocPtr/;
			$self->pidl("#if PY_MAJOR_VERSION >= 3");
			$self->pidl("$py_var = $conv_with_pycapsule;");
			$self->pidl("#else");
			$self->pidl("$py_var = $conv;");
			$self->pidl("#endif");
		} else {
